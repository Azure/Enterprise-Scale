    ...
    Context "Test UDR on Virtual Network when created or updated" -Tag "deny-subnet-udr" {
        ...
        It "Should allow compliant Virtual Network without UDR but excluded subnet" -Tag "allow-subnet-udr" {
            AzTest -ResourceGroup {
                ...
                $rule4 = New-AzNetworkSecurityRuleConfig -Name allowBHinbound-rule -Description "Allow Bastion Host Inbound" -Access Allow -Protocol * -Direction Inbound -Priority 104 -SourceAddressPrefix VirtualNetwork -SourcePortRange * -DestinationAddressPrefix VirtualNetwork -DestinationPortRange '8080,5701'
                $rule5 = New-AzNetworkSecurityRuleConfig -Name allowOnbound-rule -Description "Allow Outbound" -Access Allow -Protocol * -Direction Outbound -Priority 101 -SourceAddressPrefix * -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange *
