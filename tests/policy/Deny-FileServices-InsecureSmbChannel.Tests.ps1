...
Describe "Testing policy 'Deny-FileServices-InsecureSmbChannel'" -Tag "deny-files-channel" {
    ...
    BeforeAll {
        ...
        $definition = Get-AzPolicyDefinition | Where-Object { $_.Name -eq 'Deny-FileServices-InsecureSmbChannel' }
        New-AzPolicyAssignment -Name "TDeny-Files-channel" -Scope $mangementGroupScope -PolicyDefinition $definition

    }

    Context "Test insercure channel encryption enabled on Storage Account - File Services when created" -Tag "deny-files-channel" {

        It "Should deny non-compliant Storage Account - File Services - Insecure Channel Encryption" -Tag "deny-noncompliant-files" {
...

                {
                    ...
                    $protocolSettings = @{
                        smb = @{
                            ...
                            channelEncryption = "AES-128-CCM" # Invalid
        ...
        It "Should allow compliant Storage Account - File Services - Insecure Channel Encryption" -Tag "allow-compliant-files" {
            AzTest -ResourceGroup {
                param($ResourceGroup)
