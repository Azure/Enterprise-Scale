...
- Added new initiative and assignment to enable auditing for Trust Launch capable virtual machines which includes the following built-in policies:
  ...
  - Added new custom policy "Deploy-Private-DNS-GroupID" this policy will DINE-configure private DNS zone group to override the DNS resolution for PaaS services private endpoint, added services which supports private DNS but doesn't have built in policy exist.
