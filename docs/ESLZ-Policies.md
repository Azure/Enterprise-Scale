...
We work with - and learn from our customers and partners to ensure that we evolve and enhance the reference implementations to meet customer requirements. The primary approach of the policies as part of Azure Landing Zones is to be proactive (deployIfNotExist, and modify), and preventive (deny), and we are continuously moving these policies to built-ins.
...
All custom Azure Policy Definitions and Initiatives are the same across all 3 implementation options for Azure Landing Zones; [Terraform Module](https://aka.ms/alz/tf), [Bicep Modules](https://aka.ms/alz/bicep), [Portal Accelerator Experience](https://aka.ms/alz#azure-landing-zone-accelerator).
...
For a complete list of all custom and built-in policies deployed within an Azure Landing Zone deployment, please refer to the following [section](https://github.com/ATuckwell/ALZUpdates/blob/main/ALZ-Policies.md#what-policy-definitions-are-assigned-within-the-azure-landing-zones-custom--built-in).
...
| **Deploy-Linux-Arc-Monitoring**                                            | **Deploy-Linux-Arc-Monitoring**                                                  | `Policy Definition`, **Built-in**     | Deploys the Log Analytics agent to Linux Azure Arc machines if the agent isn't installed.                                                                                                                                                                                                                                                                                            | deployIfNotExists                   | 2.0.1   |
| **Deploy-Windows-Arc-Monitoring**                                          | **Deploy-Windows-Arc-Monitoring**                                                | `Policy Definition`, **Built-in**     | Deploys the Log Analytics agent to Windows Azure Arc machines if the agent isn't installed.                                                                                                                                                                                                                                                                                          | deployIfNotExists                   | 2.0.1   |
